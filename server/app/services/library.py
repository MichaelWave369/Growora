...
def _sync_fts(chunk: DocumentChunk):
    with engine.connect() as conn:
        conn.exec_driver_sql(
            "INSERT INTO document_chunks_fts(text, document_id, chunk_id) VALUES (?, ?, ?)",
            (chunk.text, chunk.document_id, chunk.id),
        )
        conn.commit()


def save_upload(file: UploadFile, tags: list[str], session: Session) -> Document:
    data = file.file.read()
    ...
    for page, text in pages:
        for c in _chunk_text(text):
            if not c.strip():
                continue
            chunk = DocumentChunk(document_id=doc.id, idx=idx, text=c, page=page, meta_json=json.dumps({}))
            session.add(chunk)
            session.commit()
...
def search_library(query: str, tags: list[str], limit: int = 8):
    where = ""
    params = [query, limit]
    sql = """
    ...
    """
    with engine.connect() as conn:
        rows = conn.exec_driver_sql(sql + where, tuple(params)).all()
    results = []
    with Session(engine) as session:
        for _, text, document_id, chunk_id, score in rows:
            ...
            results.append({"document": doc, "chunk_id": chunk_id, "snippet": snippet, "score": score})
    return results
